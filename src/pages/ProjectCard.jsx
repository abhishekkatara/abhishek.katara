...
const ProjectCard = ({ project: { title, description, tags, link } }) => {
  return (
    <div key={title} className="group w-full sm:w-1/2 m-4 mx-auto p-6 rounded-xl border-2 border-gray-300">
      <a href={link}>
      ...
      <div className="mt-4 mb-8 flex flex-wrap justify-center items-center gap-2">
        {tags.map((tag) => (
          <div key={tag} className="px-4 py-1 border-2 rounded-full">{tag}</div>
        ))}
      </div>
